...
#define FIREBASE_HOST "*"    //Locar onde recebe os dados da aplicação.
#define FIREBASE_AUTH "*"    //Atenticando no servidor do firebase.

//Definição dos dados da rede local
#define WIFI_SSID "*" //nome da rede wifi.
#define WIFI_PASSWORD "*"  //senha de rede wifi.
...
pinMode(pino_sinal_digital, INPUT);
  Serial.begin(9600);
  WiFi.begin(WIFI_SSID, WIFI_PASSWORD); //enviando o nome da rede e a senha para conectar  internet.
  ...
  Firebase.begin(FIREBASE_HOST, FIREBASE_AUTH); //enviando as permições pela rede 
...
void loop() {
  ...
  if(publishNewState){
    Serial.println("Publish new State"); // uma mensagem para saber que vai ocorrer a publicação dos dados
    // Obtem os dados do sensor DHT 
    if(!isnan(valor_analogico)){ // se o valor for numerico execute...
      StaticJsonBuffer<200> jsonBuffer; // Instacio a ultilização da classe jsonBuffer; 
      JsonObject& root = jsonBuffer.createObject(); // criando um objeto do mesmo tipo da classe para ser salvo no firebase
      ...
      String name = Firebase.push("sensor", root); // os dados do root são enviados
  ...
  delay(5000); // de quanto em quanto tempo sera feita a leitura do dados
  // ----- Fim
}
